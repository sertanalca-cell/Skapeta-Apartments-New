      ...
      <section id="home" className="pt-32 pb-20 px-4 relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            {/* Order Status Tracker - Like the image */}
              ...
              <div className="mb-6 w-full max-w-md mx-auto">
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-4">
                  <div className="flex items-center justify-between relative">
                    {/* Progress Line */}
                    <div className="absolute top-8 left-0 right-0 h-0.5 bg-slate-200 dark:bg-slate-700 mx-8"></div>
                    <div 
                      className="absolute top-8 left-0 h-0.5 bg-blue-500 mx-8 transition-all duration-500"
                      style={{
                        width: 
                    ...
                    {/* Steps */}
                    {[
                      { status: 'pending', label: 'Order Placed', icon: '📋', step: 0 },
                      { status: 'preparing', label: 'Preparing', icon: '👨‍🍳', step: 1 },
                      { status: 'delivering', label: 'Out for Delivery', icon: '🛵', step: 2 },
                      { status: 'delivered', label: 'Delivered', icon: '🏠', step: 3 }
                    ].map((item, idx) => {
                      const isActive = 
                      ...
                      return (
                        <div key={item.status} className="flex flex-col items-center z-10 flex-1">
                          ...
                          <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 ${
                            isActive ? 'bg-blue-500 shadow-lg scale-110' : 
                            isPast ? 'bg-blue-500' :
                          ...
                          {/* Time */}
                          {isActive && currentOrder.updated_at && (
                            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                              {new Date(currentOrder.updated_at).toLocaleTimeString('en-US', { 
                                hour: '2-digit', 
                                minute: '2-digit',
                                hour12: true 
                              })}
